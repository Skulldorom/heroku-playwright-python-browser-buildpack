...
for stack in heroku-22 heroku-24 heroku-26; do
  setup; run_native "$stack" >"$TEST_TMP/out"
  case "$stack" in heroku-22) assert_contains "$TEST_TMP/apt.log" libasound2;; *) assert_contains "$TEST_TMP/apt.log" libasound2t64;; esac
  [ -f "$BUILD_DIR/.apt/usr/lib/unpacked" ] || fail "$stack did not unpack"
  rm -rf "$TEST_TMP"
done

# Unchanged inputs reuse cache; package and stack changes invalidate it.
setup
run_native heroku-24 'one two' >/dev/null
run_native heroku-24 'one two' >"$TEST_TMP/reuse"
assert_contains "$TEST_TMP/reuse" "Reusing native dependency APT cache"
run_native heroku-24 'one three' >"$TEST_TMP/packages"
assert_contains "$TEST_TMP/packages" "refreshing APT cache"
run_native heroku-22 'one three' >"$TEST_TMP/stack"
assert_contains "$TEST_TMP/stack" "refreshing APT cache"
rm -rf "$TEST_TMP"

# Custom packages replace defaults.
setup; run_native heroku-24 'custom-a custom-b' >/dev/null
...
assert_failure env PATH="$FAKEBIN" /bin/bash "$ROOT/bin/install-native-deps" "$BUILD_DIR" "$CACHE_DIR" heroku-24 custom 2>"$TEST_TMP/error"
...
rm -rf "$TEST_TMP"
setup
assert_failure env PATH="$FAKEBIN:$PATH" TEST_TMP="$TEST_TMP" EMPTY_DOWNLOAD=1 "$ROOT/bin/install-native-deps" "$BUILD_DIR" "$CACHE_DIR" heroku-24 custom 2>"$TEST_TMP/error"
